...
def list_sources(db, user_id):
    rows = db.execute(
        select(DocChunk.source_name, func.count(DocChunk.id).label("chunks"))
        .where(DocChunk.user_id == user_id).group_by(DocChunk.source_name)
    ).all()
    return [{"source_name": r.source_name, "chunks": int(r.chunks)} for r in rows]


def delete_source(db, user_id, source_name):
    db.execute(delete(DocChunk).where(DocChunk.user_id == user_id,
