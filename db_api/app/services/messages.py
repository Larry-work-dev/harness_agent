...
def add_message(db, conversation_id, role, content, sources=None):
    db.add(Message(conversation_id=conversation_id, role=role, content=content, sources=sources))
...
def list_messages(db, conversation_id) -> list:
    ...
    return [{"id": m.id, "role": m.role, "content": m.content,
             "sources": m.sources, "created_at": m.created_at} for m in rows]
